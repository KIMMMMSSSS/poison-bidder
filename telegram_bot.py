...
# 순서: 링크개수 → 적립금선할인 → 카드할인 → 기본할인율 → 최소수익
(WAITING_LINK_COUNT, WAITING_POINTS_USE, WAITING_POINTS_RATE, 
 WAITING_CARD_USE, WAITING_CARD_INPUT,
 WAITING_DISCOUNT, WAITING_PROFIT) = range(7)
...
class BiddingBot:
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # 사용자 ID 표시 (디버깅용)
        await update.message.reply_text(f"🔍 당신의 텔레그램 ID: `{user.id}`", parse_mode='Markdown')
        
        if not self.is_authorized(user.id):
            await update.message.reply_text(f"{self.config['messages']['unauthorized']}\n\n💡 관리자에게 이 ID를 알려주세요: `{user.id}`", parse_mode='Markdown')
            return
...
시작하려면 `/auto` 명령어를 사용하세요!
    ...
    async def auto_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        user = update.effective_user
        logger.info(f"Auto command from user {user.id}: {user.first_name}")
        
        if not self.is_authorized(user.id):
            logger.warning(f"Unauthorized auto command from user {user.id}")
        ...
        # 파라미터 파싱
        args = context.args
        logger.info(f"Auto command args: {args}")
        ...
        if not keywords:
            logger.info("No keywords provided, showing help message")
        ...
        logger.info(f"Starting auto command with site: {site}, keywords: {keywords}")
        ...
        keyboard = [
            [
                InlineKeyboardButton("🚀 빠른 테스트 (50개)", callback_data="links_50"),
                InlineKeyboardButton("📦 표준 (100개)", callback_data="links_100")
            ],
            [
                InlineKeyboardButton("📊 많이 (200개)", callback_data="links_200"),
                InlineKeyboardButton("⚙️ 사용자 지정", callback_data="links_custom")
            ]
        ]
        ...
        await update.message.reply_text(
            ...
            f"사이트: {site.upper()}\n\n"
            f"몇 개의 링크를 추출하시겠습니까?\n\n"
            f"💡 링크가 많을수록 시간이 오래 걸립니다.\n"
            f"    테스트는 50개로 충분합니다.",
            reply_markup=reply_markup,
            parse_mode='Markdown'
        )
        
        return WAITING_LINK_COUNT
...
진행 단계: {self.current_task.get('stage', 'N/A')}
    ...
    async def profit_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            settings_msg += f"🔗 추출 링크 수: {context.user_data.get('target_links', 100)}개\n\n"
            # 적립금 선할인 정보
            ...
            # callback_data는 간단히 (64바이트 제한)
            # 모든 설정은 이미 context.user_data에 저장됨
            
            keyboard = [
                [
                    InlineKeyboardButton("✅ 시작", callback_data="auto_start_confirm"),
    ...
    async def link_count_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """링크 개수 입력 처리 (사용자 지정)"""
        try:
            # 입력값 파싱
            text = update.message.text.strip()
            ...
            # 저장
            context.user_data['target_links'] = link_count
            
            # 적립금 선할인 사용 여부 질문으로 이동
            keyboard = [
                [
            ...
            await update.message.reply_text(
                f"🔗 링크 {link_count}개로 설정되었습니다.\n\n"
                f"💳 **무신사 적립금 선할인**\n\n"
                f"무신사 적립금 선할인을 사용하시겠습니까?\n\n"
                f"📄 멤버십 등급별 적립률:\n"
                f"• 브론즈: 2%\n"
                f"• 실버: 3%\n"
                f"• 골드: 4%\n"
            ...
            logger.error(f"링크 개수 입력 처리 오류: {e}")
            await update.message.reply_text(
                "⚠️ 처리 중 오류가 발생했습니다.\n"
                "다시 입력해주세요."
            )
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # 링크 개수 처리
        if data.startswith("links_"):
            if data == "links_custom":
                # 사용자 지정 입력 요청
                ...
                # 미리 정의된 개수 선택
                link_count = int(data.replace("links_", ""))
                context.user_data['target_links'] = link_count
        ...
        if data == "points_use_yes":
            ...
            # context.user_data에서 모든 설정 가져오기
            site = context.user_data.get('site', 'musinsa')
            keywords = context.user_data.get('keywords', [])
            custom_discount_rate = context.user_data.get('discount_rate')
            custom_min_profit = context.user_data.get('min_profit')
            points_rate = context.user_data.get('points_rate', 0)
            card_discount = context.user_data.get('card_discount')
            target_links = context.user_data.get('target_links', 100)
            ...
            if data.startswith("auto_start_with_discounts_"):
                # 새로운 형식: auto_start_with_discounts_{site}_{keywords}_{discount}_{profit}_{points_rate}_{use_card}_{target_links}
                parts = data.replace("auto_start_with_discounts_", "").split("_")
                
                target_links = 100  # 기본값
                ...
                if len(parts) >= 7:
                    ...
                    try:
                        # 설정값 파싱
                        target_links = int(parts[-1])
                        use_card = parts[-2] == '1'
                        points_rate_value = float(parts[-3])
                        custom_min_profit = int(parts[-4])
                        custom_discount_rate = float(parts[-5])
                        
                        # 키워드 파싱
                        keywords_part = "_".join(parts[1:-5])
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                WAITING_LINK_COUNT: [
                    ...
                    MessageHandler(filters.TEXT & ~filters.COMMAND, self.link_count_handler)
                ],
